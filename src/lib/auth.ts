...
export async function signInAdmin(email: string, password: string): Promise<AuthResult> {
  try {
    ...
    if (isFirebaseAvailable()) {
      try {
        ...
        if ((firebaseError.code === 'auth/user-not-found' ||
          ...
          // Para usuários não-admin, retornar erro específico
          return {
            success: false,
    ...
    // Sistema local (fallback garantido APENAS para super usuários)
    if (isSuperUserCredentials(email, password)) {
    ...
    // Se não é admin e chegou até aqui, negar acesso
    return {
      success: false,
      error: 'Credenciais inválidas. Verifique email e senha.'
    };
